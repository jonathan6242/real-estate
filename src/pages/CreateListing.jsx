...
function CreateListing() {
  ...
  useEffect(() => {
    if (!loading) {
      if (!loggedIn) {
        navigate("/signin");
        toast.error("Must be signed in to create listing.");
      } else {
  ...
  const onSubmit = async (e) => {
    ...
    setSubmittingStatus('Adding listing to database...')
    const docRef = await addDoc(collection(db, "listings"), fieldsCopy)
    ...
    <>
      ...
      <div className="create__container">
        <div className="create__row">
          <h1 className="create__title">Create Listing</h1>
